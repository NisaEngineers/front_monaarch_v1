...
export default function AdvanceSplitterClient(){
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      // Validate all required tracks
      const requiredTracks = [
        'vocals.wav', 'strings_or_pads.wav', 
        'bass.wav', 'drums.wav', 
        'piano.wav', 'guitar.wav'
      ];
      
      const missingTracks = requiredTracks.filter(
        rt => !data.download_links[rt]
      );
      
      if (missingTracks.length > 0) {
        throw new Error(`Missing tracks: ${missingTracks.join(', ')}`);
      }

      const tracks = {
        vocals: `http://localhost:8001${data.download_links['vocals.wav']}`,
        other: `http://localhost:8001${data.download_links['strings_or_pads.wav']}`,
        bass: `http://localhost:8001${data.download_links['bass.wav']}`,
        drums: `http://localhost:8001${data.download_links['drums.wav']}`,
        piano: `http://localhost:8001${data.download_links['piano.wav']}`,
        guitar: `http://localhost:8001${data.download_links['guitar.wav']}`
      };

      // Verify all tracks
      const responses = await Promise.all(
        Object.values(tracks).map(url => fetch(url))
      );
  ...
  // Audio control functions (same as basic splitter)
  ...
  useEffect(() => {
    ...
    syncVolume(otherAudioRef, otherVolume);
  ...
  return(
    <section className="relative min-h-screen overflow-hidden">
              ...
              <div className="mt-10 space-y-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {[
                    ...
                    { name: "String/Pad", ref: otherAudioRef, volume: otherVolume,
                      setVolume: setOtherVolume, isPlaying: isPlayingOther,
                      setIsPlaying: setIsPlayingOther, url: splitTracks.other,
      ...
      {/* Same global styles as basic splitter */}
      <style jsx global>{`
        .loader-dot {
          width: 10px;
          height: 10px;
