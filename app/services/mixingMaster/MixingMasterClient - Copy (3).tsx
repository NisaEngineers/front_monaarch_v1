...
export default function MixingMasterClient() {
  ...
  const [highCutoff, setHighCutoff] = useState(2000); // default 2000 Hz
  const [lowCutoff, setLowCutoff] = useState(200); // default 200 Hz
  const [decibelLevel, setDecibelLevel] = useState(-18); // default -18 dB
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      formData.append("freq_low", String(lowCutoff));
      formData.append("freq_high", String(highCutoff));
      formData.append("reduction_db", String(decibelLevel));
          ...
          <div className="py-12 md:py-20">
            ...
            {uploaded && !processing && masteredAudioURL && (
              <div className="mt-10 flex flex-col space-y-12">
                {/* Audio Player Map */}
                {[{ title: "Original Audio", url: originalAudioURL, ref: originalAudioRef, volume: originalVolume, setVolume: setOriginalVolume, isPlaying: isPlayingOriginal, isPaused: isOriginalPaused, toggle: toggleOriginalPlayPause, bgImage: "/original_m.png", color: "blue" },
                  { title: "Mastered Audio", url: masteredAudioURL, ref: masteredAudioRef, volume: masteredVolume, setVolume: setMasteredVolume, isPlaying: isPlayingMastered, isPaused: isMasteredPaused, toggle: toggleMasteredPlayPause, bgImage: "/mixing_bg.png", color: "green" }].map((audio, idx) => (
                  <div
                    key={idx}
                  ...
                  >
                    ...
                    <div className="flex flex-col items-center">
                      <div className="flex items-center space-x-4">
                        <button onClick={audio.toggle} className="play-button focus:outline-none">
                          <svg xmlns="http://www.w3.org/2000/svg" className={`h-12 w-12 text-${audio.color}-500`} fill="currentColor" viewBox="0 0 24 24">
                            {audio.isPaused ? <path d="M5 3.5v17l14-8.5-14-8.5z" /> : <path d="M6 4h4v16h-4v-16zm8 0h4v16h-4v-16z" />}
                          </svg>
                        </button>
                        <input
                          type="range"
                      ...
                      <audio ref={audio.ref} src={audio.url!} onEnded={() => { audio.setVolume(0); audio.isPaused ? setIsPlayingOriginal(false) : setIsPlayingMastered(false); }} />
                    </div>
